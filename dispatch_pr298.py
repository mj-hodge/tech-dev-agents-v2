...
async def needs_info_story(story_id: str, request: Request):
    ...
    # STORY-738: enforce 64 KB cap on question_text
...
async def get_question(story_id: str, request: Request):
...
async def answer_story(story_id: str, body: AnswerRequest, request: Request):
    ...
    try:
        row = await db_svc.answer_needs_info(story_id, body.answer, body.operator)
    except NotFoundError:
        raise HTTPException(404, f"{story_id} not found")
    except InvalidTransitionError as exc:
        raise HTTPException(409, str(exc))
